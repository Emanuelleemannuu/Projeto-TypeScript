...
export function converter() {
    tarefas.sort((objetoA, objetoB) => {
        ...
        let dateA: Date = new Date(`${dataA[2]}-${dataA[1]}-${dataA[0]}`)
        let dateB: Date = new Date(`${dataB[2]}-${dataB[1]}-${dataB[0]}`)
...
export function ordenar() {
    let ordenar: number = Number(prompt(`
        1 - Vencimento
        2 - Prioridade
        3 - Criação
    ...
    if (ordenar === 1) {
        ...
        tarefas.sort((a, b) => {
            let prioridadeA: string = a.prioridade.toLowerCase()
            let prioridadeB: string = b.prioridade.toLowerCase()
...
export function marcarConcluida() {
    ...
    if (procurarEmTarefas) {
        ...
        let index = tarefas.findIndex((tarefa) => tarefa.titulo === marcar)
        if (index !== -1){
